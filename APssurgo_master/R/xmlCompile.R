...
if(FALSE){
  data = soils
  site_name = soils$Average_Soil$coords$SiteName 
...
xmlCompile <- function(data,site_name,crops,coords) {
  ...
  for(i in 1:length(data)){
    ...
    ## <Soil name="Default">
    Soil <- newXMLNode("Soil", attrs = list(name=paste0(site_name, " - (",names(data)[i]," ",round(unique(data[[i]]$area)*100),"% of AOI)")),
                       parent = folder)
    ...
    ### <Region>Story</Region>
    county <- capitalize(latlong2county(lat=coords[,2], long=coords[,1]))
    ...
    xmlValue(Latitude) <- round(coords[,2])
    
    ### <Longitude>-0</Longitude>
    Longitude <- newXMLNode("Longitude",parent = Soil)
    xmlValue(Longitude) <- round(coords[,1])
