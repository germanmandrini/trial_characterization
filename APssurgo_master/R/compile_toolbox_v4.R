...
make_toolbox <- function(data_soils,badge_name,crops) {
  ...
  for(i in 1:length(unique(data_soils$farm))){
    # i=1
    site_name = unique(data_soils$farm)[i]
    data_site <- data_soils[farm == site_name]
    field_name = unique(data_site$field)

    site <- newXMLNode("folder", attrs = list(farm = site_name,
                                              field = field_name), parent = folder)
    
    # if(is.null(data_site)) next

    for(n in 1:length(unique(data_site$name))){
      # n=1
      
      soil_name <- unique(data_site$name)[n]
      data_soil <- data_site[name == soil_name]

      print(paste0(toupper(site_name), ' ',
                   soil_name,": ",
                   round(mean(data_soil$area_pct),2)*100,
                   "% of AOI"))
      


      ## <Soil name="Default">
      # Soil <- newXMLNode("Soil", attrs = list(name=paste0(soil_name,"-",round(mean(data_soil$area_pct),2)*100,"% of AOI")),
      #                    parent = site)
      Soil <- newXMLNode("Soil", attrs = list(soil= soil_name),
                         parent = site)
      ...
      xmlValue(SoilType) <- 'Average_Soil'

      ### <Region>Story</Region>
      county <- capitalize(latlong2county(lat=mean(data_site$Y), long=mean(data_site$X)))
      ...
      xmlValue(Comments) <- paste0("This soil was created with data from SSURGO Database, downloaded in R via the FedData package and parameters set using the approach described by Archontoulis et al. (2014, Agron. J. 106(3):1025-1040). This soil type represents ",
                                   round(mean(data_site$area_pct)*100),"% of AOI.")
      ### <Water>
      ...
      xmlValue(DiffusConst) <-  mean(data_site$DiffusConst)

      #### <DiffusSlope>
      DiffusSlope <- newXMLNode("DiffusSlope",parent = SoilWater)
      xmlValue(DiffusSlope) <-  mean(data_site$DiffusSlope)

      #### <Salb>
      Salb <- newXMLNode("Salb",parent = SoilWater)
      xmlValue(Salb) <- mean(data_site$Salb)

      #### <CN2Bare>
      CN2Bare <- newXMLNode("CN2Bare",parent = SoilWater)
      xmlValue(CN2Bare) <- (round(mean(data_site$CN2),0))

      #### <CNRed>
      CNRed <- newXMLNode("CNRed",parent = SoilWater)
      xmlValue(CNRed) <- (mean(data_site$CNRed))

      #### <CNCov>
      CNCov <- newXMLNode("CNCov",parent = SoilWater)
      xmlValue(CNCov) <- (mean(data_site$CNCov))
      ...
      xmlValue(RootCN) <- mean(round(data_site$RootCN))

      #### <RootWt>
      RootWt <- newXMLNode("RootWt",parent = SoilOrganicMatter)
      xmlValue(RootWt) <- mean(round(data_site$RootWt))

      #### <SoilCN>13</SoilCN>
      SoilCN <- newXMLNode("SoilCN",parent = SoilOrganicMatter)
      xmlValue(SoilCN) <- mean(round(data_site$SoilCN))

      #### <EnrACoeff>
      EnrACoeff <- newXMLNode("EnrACoeff",parent = SoilOrganicMatter)
      xmlValue(EnrACoeff) <- mean(round(data_site$EnrAcoeff,2))

      #### <EnrBCoeff>
      EnrBCoeff <- newXMLNode("EnrBCoeff",parent = SoilOrganicMatter)
      xmlValue(EnrBCoeff) <- mean(round(data_site$EnrBcoeff,2))
