...
compile_toolbox <- function(data_soils,badge_name, path,crops) {
  ...
  for(tile_n in sort(unique(data_soils$id_tile))){
    # tile_n <- sort(unique(data_soils$id_tile))[2]

    data_tile <- data_soils[id_tile == tile_n]

    site <- newXMLNode("folder", attrs = list(name=paste('tile', tile_n, sep = '_')), parent = folder)

    soil_seq <- sort(as.numeric(unique(data_tile$mukey)))
    for(soil_n in soil_seq){
      # soil_n = soil_seq[1]
  
      data_soil <- data_tile[mukey == soil_n]
  
      print(paste0('tile: ', tile_n, ' / mukey: ', soil_n))
  ...
  doc <- xmlChildren(folder, useInternalNodes = TRUE)
  length(doc)
  sapply(doc, xmlGetAttr, "name")

  doc_onefarm <- xmlChildren(doc[[1]], useInternalNodes = TRUE)
  length(doc_onefarm)
  sapply(doc_onefarm, xmlGetAttr, "name")
