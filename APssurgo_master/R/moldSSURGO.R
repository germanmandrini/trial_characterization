...
moldSSURGO <- function(x, soilLayer_breaks = c(0,10,20,30,100)){
  ...
  majcomp %>%
    ...
    mutate(watertable_g = ifelse(is.na(watertable),NA,watertable + 50)) -> h #mutate(watertable_g = watertable + 50) -> h 
...
h %>%
  ...
  if(any(h2$maxdepth < max(soilLayer_breaks))) {
    ...
    h <- rbind(h,h2)
    h <- h[order(h$name, h$bottom),]
  }
  h <- dplyr::filter(h, name == 'Crider_PcC3')
  h %>%
    select(-maxdepth) %>%
