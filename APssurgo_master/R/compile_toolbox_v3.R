...
compile_toolbox <- function(data_soils,badge_name,crops) {
  ...
  folder <- newXMLNode("folder", attrs = list(version="36",
                                              creator="Apsim 7.10",
                                              name= badge_name))
  for(i in 1:length(unique(data_soils$SiteName))){
    # i=2
    site_name = unique(data_soils$SiteName)[i]

    site <- newXMLNode("folder", attrs = list(version="36",
                                                creator="Apsim 7.10",
                                                name=site_name), parent = folder)
    data_site <- data_soils[SiteName == site_name]
    ...
    #for(n in 1:length(data_site)){
      ...
      ## <Soil name="Default">
      Soil <- newXMLNode("Soil", attrs = list(name=paste0(site_name,"-",round(mean(data_site$area_pct),2)*100,"% of AOI")),
                         parent = site)
