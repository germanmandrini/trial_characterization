...
if(test){
h = x2
by_soil = input_n$myBy_soil
soilLayer_breaks = c(0, 10,20,30,100)#soil_layers
}
calcSSURGO_gm <- function(h, by_soil = TRUE, soilLayer_breaks = c(10,20,30,100)){
  h <- data.frame(h)
  ...
  for(i in 1:length(unique(h$name))){
    ...
    horizon <- h[h$name == unique(h$name)[i],] %>% dplyr::select(-layer)
    ...
    horizon$layer[is.na(horizon$layer)] <- max(horizon$layer[!is.na(horizon$layer)])
    
    # horizon %>%
    #   dplyr::select(-hydrogroup) %>%
    #   group_by(layer) %>%
    #   mutate(thick = 10 + (max(center)-min(center))*10) %>% #transform to mm
    #   summarise_all("mean") -> horizon #summarize by layer
    
    
    horizon2 <- data.table(horizon) %>% .[,-('hydrogroup')]

    part1 <- horizon2[,.(name = unique(name),
                         top = min(center)-1,
                      bottom = max(center)), by = layer] %>% .[,thick := (bottom - top)*10]

    cols = setdiff(names(horizon2), c('name','layer','top', 'bottom', 'thick'))
    part2 <- horizon2[, lapply(.SD, mean), by = layer, .SDcols = cols]
    
    horizon3 <- merge(part1, part2, by = 'layer')
    
     
    
    # Save df into list ###########
    
    soils.dt <- rbind(soils.dt, horizon3[,-c('center','slope_code','slope')])
  ...
  if(by_soil) {
    ...
    final_depth <- sum(soils.dt[,.(bottom = max(bottom),
                                    area = mean(area)), by = name] %>% .[,w_bottom := (bottom * area)] %>% .[,w_bottom])
    
    # sand_layer3 <- soils.dt2[layer == 3, c('name', 'sand', 'area')] %>% .[, sand_w := sand * (area/sum(area))] %>% .[,sand_w] %>% sum()
    # sand_layer4 <- soils.dt2[layer == 4, c('name', 'sand', 'area')] %>% .[, sand_w := sand * (area/sum(area))] %>% .[,sand_w] %>% sum()
     
    cols_sum <- setdiff(names(soils.dt), c('name', 'layer'))
    
    average_soil <- soils.dt[, lapply(.SD, weighted.mean, w =area, na.rm=TRUE), by=layer, .SDcols=cols_sum ]
    # round(average_soil[layer == 3, sand],3) == round(sand_layer3,3)
    # round(average_soil[layer == 4, sand],3) == round(sand_layer4,3)
     
    #Fix the final depth
    average_soil <- average_soil[top <= final_depth]
    average_soil[bottom > final_depth, bottom := final_depth]
    average_soil[,thick := round((bottom-top)*10)]
    soils.dt <- average_soil
...
#calcSSURGO_gm(x2,by_soil = TRUE)
test <- calcSSURGO_gm(x2,by_soil = TRUE)
