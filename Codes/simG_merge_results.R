...
for(mukey_n in mukey_seq){
  ...
  #SAVE THE OUTPUT
  file_output_name <- paste('./trial_characterization_box/Data/','yc_output','_',batch_n,'_', water_n, '/', id10_n,"_",mukey_n, '.rds', sep = '')
...
if(FALSE){
  ...
  # COMPARE THE YIELD
  ic_files <- list.files(paste0('./trial_characterization_box/Data/initial_conditions'), full.names = TRUE)
  ...
  yc_files <- list.files(paste0('./trial_characterization_box/Data/yc_output'), full.names = TRUE)
